...
public class Kinematics {
	...
	private double f1(double x, double y, double phi, double y1, double y2) {
		...
		return h3 * Math.sin(phi) + Math.abs(h / (tanGamma + tanLambda))
				* (Math.sin(phi2) - Math.sin(phi1) + tanGamma * Math.cos(phi1) - tanLambda * Math.cos(phi2));
	}
	...
	public Point computePoint(double x, double y) {
		...
		while (true) {
			...
			// deltaY1, deltaY2, jacobian, jacobianInverse);
		...
		double left = Math.sqrt(Math.pow(x - h * Math.cos(phi1), 2) + Math.pow(y + y1 - h * Math.sin(phi1), 2))
				- r * tanGamma + r * gamma;
		double right = Math
				.sqrt(Math.pow(machineWidth - x + h * Math.cos(phi2), 2) + Math.pow(y + y2 - h * Math.sin(phi2), 2))
				- r * tanLambda + r * lambda;

		// System.out.format("Final: (phi: %3.2f) (y1: %3.2f) (y2: %3.2f); (phi1: %3.2f)
		// (phi2: %3.2f)\n", phi, y1, y2, phi1, phi2);
